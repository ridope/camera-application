...
uint8_t ridope_otsu(const uint8_t *img_in, uint8_t *img_out, uint8_t *threshold, size_t height, size_t width)
{
	...
	*threshold = 0;
	...
	double var_class, var_max, sum, sumB, q1, q2, u1, u2 = 0;
...
uint8_t ridope_edge_tracking(uint8_t *img_in_out, uint8_t high_threshold, uint8_t low_threshold, size_t height, size_t width)
{
	...
	for(int i = 0; i < weak_edges_size; i += 2)
	{
		...
		for(int x = -1; x <1; x++)
		{
			for(int y = -1; y < 1; y++)
			{
				int img_x = get_updated_index(weak_edge_x-x, 0, width);
				int img_y = get_updated_index(weak_edge_y-y, 0, height);

				if(img_in_out[width * img_x + img_y] == 255)
				{
					is_connected = 1;
				}
			}
		...
		if(is_connected)
		{
			img_in_out[width * weak_edge_x + weak_edge_y] = 255;
